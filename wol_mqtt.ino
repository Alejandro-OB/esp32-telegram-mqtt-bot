...
void conectarWiFi() {
  ...
  Serial.println("\n✅ WiFi conectado");
  mqttClient.publish("mqtt/respuesta/reconexion_wifi", ("✅ Reconexión WiFi completada. IP: " + WiFi.localIP().toString()).c_str());
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "mqtt/pc/encender" && mensaje == "true") {
    if (enviarWOL()) {
      ...
      delay(5000); // esperar a que arranque
      bool encendido = Ping.ping(ipPC);
      if (encendido) mqttClient.publish("mqtt/respuesta/verificar", "✅ PC ENCENDIDO por WOL");
      else mqttClient.publish("mqtt/respuesta/verificar", "⚠️ WOL enviado pero el PC no respondió");
    } else {
    ...
    if (client.connect(ipPC, PUERTO_FLASK)) {
      ...
      delay(10000);
      if (Ping.ping(ipPC)) mqttClient.publish("mqtt/respuesta/reiniciar_pc", "♻️ PC reiniciado correctamente.");
...
void setup() {
  ...
  conectarMQTT();
  iniciarServidorOTA();
  mqttClient.publish("mqtt/respuesta/esp32_reinicio_detectado", "🔄 ESP32 reiniciado (encendido o reseteado).");
...
void loop() {
  ...
  // Verificar reconexión WiFi
  static unsigned long ultimoCheckWiFi = 0;
  if (millis() - ultimoCheckWiFi > 10000) {
    ultimoCheckWiFi = millis();
    if (WiFi.status() == WL_CONNECTED && !yaNotificadoReconexion) {
      yaNotificadoReconexion = true;
      mqttClient.publish("mqtt/respuesta/reconexion_wifi", "🔄 ESP32 se ha reconectado a WiFi.");
    }
  }
}
